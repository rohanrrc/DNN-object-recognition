...
class NeuronLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  NeuronLayerTest()
      : blob_bottom_(new Blob<Dtype>(50, 128, 128, 128)), //num channels height width   2 3 4 5       50  128  13  13
...
TYPED_TEST(NeuronLayerTest, TestReLU) {
  ...
  printf("benchmarking RELU ----------------------------------------------: %f Gflops\n",this->blob_bottom_vec_[0]->count()/(t0 * 1000000000));
  // Now, check values
  const Dtype* bottom_data = this->blob_bottom_->cpu_data();
  const Dtype* top_data = this->blob_top_->cpu_data();
